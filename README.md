<img src="https://raw.githubusercontent.com/sayomiyori/sayomiyori/main/github-banner-1280x320.png" width="1280" alt="Banner">

# Матвей Бурик

### Python · C# · C++ · Frontend · Junior Developer  
**Симферополь** · Telegram-боты, десктоп, лендинги, системные утилиты

[![Telegram](https://img.shields.io/badge/Telegram-@sayomiyori-26A5E4?style=flat-square&logo=telegram)](https://t.me/sayomiyori)
[![VK](https://img.shields.io/badge/VK-sayzeromi-0077FF?style=flat-square&logo=vk)](https://vk.com/sayzeromi)
[![YouTube](https://img.shields.io/badge/YouTube-@sayomiyori-FF0000?style=flat-square&logo=youtube)](https://youtube.com/@sayomiyori)
[![GitHub](https://img.shields.io/badge/GitHub-sayomiyori-181717?style=flat-square&logo=github)](https://github.com/sayomiyori)

---

## 🛠 Стек

| Backend | Desktop | Frontend | Инструменты |
|---------|---------|----------|-------------|
| **Python** — aiogram, asyncio, SQLite, pytest | **C#** — WPF/MVVM, SQL Server | **HTML5**, **CSS3**, **Vanilla JS** — mobile-first, GitHub Pages | Git · Figma · Blender · Aseprite · After Effects |
| **C++** — WinAPI, low-level хуки, многопоточность |  |  |  |

---

## 📦 Ключевые проекты

| Проект | Стек | Описание | Демо |
|--------|------|----------|------|
| [**BodyTellingBot**](https://github.com/sayomiyori/BodyTellingTelegramBot) | Python · aiogram · SQLite | Бот для фитнес-клуба: опросы, подбор тренировок, достижения | — |
| [**HotkeyManager**](https://github.com/sayomiyori/HotkeyManager) | C++ · WinAPI | Менеджер горячих клавиш с low-level перехватом ввода | — |
| [**ProcessMonitor**](https://github.com/sayomiyori/ProcessMonitor) | C++ · WinAPI | Системный монитор процессов, аналог Process Explorer | — |
| [**NailLanding**](https://github.com/sayomiyori/NailLanding) | HTML · CSS · JS | Лендинг мастера маникюра | [▶ Live](https://sayomiyori.github.io/NailLanding/) |
| [**CakesLanding**](https://github.com/sayomiyori/cakesLanding) | HTML · CSS · JS | Лендинг кондитера | [▶ Live](https://sayomiyori.github.io/cakesLanding/) |

---

## 📈 GitHub

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=sayomiyori&show_icons=true&theme=default&count_private=true" alt="GitHub Stats" width="49%" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=sayomiyori" alt="Streak" width="49%" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sayomiyori&layout=compact&theme=default" alt="Top Langs" />
</p>

---

## 📬 Связаться

Сейчас углубляюсь в backend на **Python** и десктоп на **C#**. Открыт к пет-проектам и коллаборациям.

👉 **Написать:** [Telegram](https://t.me/sayomiyori) · [VK](https://vk.com/sayzeromi)
